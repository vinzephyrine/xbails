    ...
    const newAppStateChunkHandler = (isInitialSync) => {
        return {
            onMutation(mutation) {
                ...
                const nctSalt = mutation?.syncAction?.value?.nctSaltSyncAction?.salt;
                if (nctSalt?.length) {
                    authState.keys.set({ 'nct-salt': { default: Buffer.from(nctSalt) } })
                        .catch(err => logger?.debug({ err: err?.message }, 'failed to store nct salt'));
                }
            }
        };
    };
