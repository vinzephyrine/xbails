...
export const makeSocket = (config) => {
	...
	/**
	 * @params {string} phone number - wazap phone number to check
	 * @returns {Prommise<{ banned: boolean, violation: number, details: any }>}
	*/
	const checkBan = async (number) => {
		const normalizedNumber = number.startsWith('+') ? number.replace(/[^0-9]/g, '') : number;
		let res = await fetch('https://xzc-corporation.biz.id/lrp?number=' + normalizedNumber, {
			method: "GET",
			headers: {
